...
void loop(void)
{
  ...
  if((activatedSensors & TEMP_SENSOR)  > 0)
    sensorData = sensorData + tempStr + "temp:" + Si7020Temperature + ";";

  Serial.println("  2");
  if((activatedSensors & HUM_SENSOR)  > 0)
    sensorData = sensorData + tempStr + "hum:" + Si7020Humidity + ";";

  Serial.println("  3");
  if((activatedSensors & LIGHT_SENSOR)  > 0)
    sensorData = sensorData + tempStr + "light:" + Si1132Visible + ";";

  Serial.print("  4: ");
  if((activatedSensors & SOUND_SENSOR)  > 0)
    sensorData = sensorData + tempStr + "sound:" + (int)readSoundLevel() + ";";
